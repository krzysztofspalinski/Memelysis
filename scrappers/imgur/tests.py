...
class ImgurScrapperTests(TestCase):
    ...
    def test_obtain_data_from_imgur_returns_data_in_correct_timestamp_window(self):
        if len(self.images) > 0:
            min_timestamp = min(self.images, key=lambda x: x.get("additional_data", {}).get("date")).get("additional_data", {}).get("date")
            max_timestamp = max(self.images, key=lambda x: x.get("additional_data", {}).get("date")).get("additional_data", {}).get("date")
            self.assertIn(min_timestamp, range(self.start_timestamp, self.end_timestamp))
            self.assertIn(max_timestamp, range(self.start_timestamp, self.end_timestamp))

    def test_obtain_data_from_imgur_downloads_images(self):
        self.assertTrue(os.path.exists(os.path.join(sys.path[0], "tmp", "imgur")))

    def test_obtain_data_from_imgur_creates_json_file(self):
        self.assertTrue(len([x for x in os.listdir(os.path.join(sys.path[0], "tmp")) if x.startswith("imgur") and x.endswith(".json")]) > 0)

    def test_obtain_data_from_imgur_creates_log(self):
        self.assertTrue(len([x for x in os.listdir(os.path.join(sys.path[0], "tmp")) if x.startswith("imgur") and x.endswith(".txt")]) > 0)
